...
export class Exporter {
     ...
     */
    generateDataYaml(trainCount, validCount, testCount = 0) {
...
# Dataset info`;
    ...
    /**
     * Generate a complete YOLO dataset with train/valid/test splits
     */
    async generateYOLODataset(generateFrame, trainCount, validCount, testCount = 0, onProgress = null) {
        const zip = new JSZip();

        let trainImages = null, trainLabels = null;
        let validImages = null, validLabels = null;
        let testImages = null, testLabels = null;

        if (trainCount > 0) {
            trainImages = zip.folder('train/images');
            trainLabels = zip.folder('train/labels');
        }
        if (validCount > 0) {
            validImages = zip.folder('valid/images');
            validLabels = zip.folder('valid/labels');
        }
        if (testCount > 0) {
            testImages = zip.folder('test/images');
            testLabels = zip.folder('test/labels');
        }

        const total = trainCount + validCount + testCount;
        let current = 0;

        for (let i = 0; i < trainCount; i++) {
            const { imageData, labels } = await generateFrame();
            const filename = `image_${String(i).padStart(6, '0')}`;
            trainImages.file(`${filename}.png`, imageData, { base64: true });
            trainLabels.file(`${filename}.txt`, labels);
            current++;
            if (onProgress) onProgress(current, total, 'train');
        }

        for (let i = 0; i < validCount; i++) {
            const { imageData, labels } = await generateFrame();
            const filename = `image_${String(i).padStart(6, '0')}`;
            validImages.file(`${filename}.png`, imageData, { base64: true });
            validLabels.file(`${filename}.txt`, labels);
            current++;
            if (onProgress) onProgress(current, total, 'valid');
        }

        for (let i = 0; i < testCount; i++) {
            const { imageData, labels } = await generateFrame();
            const filename = `image_${String(i).padStart(6, '0')}`;
            testImages.file(`${filename}.png`, imageData, { base64: true });
            testLabels.file(`${filename}.txt`, labels);
            current++;
            if (onProgress) onProgress(current, total, 'test');
        }

        zip.file('data.yaml', this.generateDataYaml(trainCount, validCount, testCount));
    ...
    toScreenPosition(obj) {
        const yolo = this.toYOLOFormat(obj);
        if (!yolo) return null;

        const minX = yolo.x_center - yolo.width / 2;
        const maxX = yolo.x_center + yolo.width / 2;
        const minY = yolo.y_center - yolo.height / 2;
        const maxY = yolo.y_center + yolo.height / 2;
